    ...
    <div class="w-100 p-3">
        ...
        <nav aria-label="breadcrumb mt-3 ">
            <ol class="breadcrumb p-3">
                ...
                <li class="breadcrumb-item previous-page"><a href="/education"></a></li>
                <li class="breadcrumb-item active fw-bold" aria-current="page">Choose Difficulty</li>
            </ol>
        </nav>
        ...
        <div class="d-flex mb-3">
            ...
            <div class="ms-auto">
                <button id="toggleBlockedButton" class="btn btn-add-level text-white fw-bold" data-bs-toggle="modal" data-bs-target="#AddEducationLevel">Add Education Level</button>
            </div>
        </div>
        ...
        <table id="membersTable" class="table table-striped table-bordered my-3">
            <thead>
                <tr>
                    ...
                    <th class="text-center align-middle">Education Level</th>
...
<div class="modal fade" id="AddEducationLevel" tabindex="-1" aria-labelledby="AddEducationLevel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Add Education Level</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
            <label for="EducationLevelName" class="form-label">Education Level:</label>
            <input type="text" class="form-control" id="EducationLevelName" aria-describedby="emailHelp">
            <div id="emailHelp" class="form-text">Kindly  input the education level name here.</div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onclick="AddEducationLevel()">Save</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="EditEducationLevel" tabindex="-1" aria-labelledby="EditEducationLevel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Update Education Level</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
            <label for="EditEducationLevelName" class="form-label">Education Level:</label>
            <input type="hidden" id="EditEducationLevelId"> <!-- Hidden input field for ID -->
            <input type="text" class="form-control" id="EditEducationLevelName" aria-describedby="emailHelp">
            <div id="emailHelp" class="form-text">Kindly  input the education level name here.</div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onclick="UpdateEducationLevel()">Save</button>
      </div>
    </div>
  </div>
</div>
...
<script>
    ...
    var page_id = urlParts[urlParts.length - 1];
    ...
    function handlePageData() {
        ...
        fetch(`/api/page-education-topic/${page_id}`)
            .then(response => response.json())
            .then(data => {
                console.log(data);
                // Update breadcrumb with name_topic
                const breadcrumbItem = document.querySelector('.breadcrumb-item.previous-page a');
    ...
    function handleTableData() {
        ...
        // console.log('This is page ' + page_id);
        ...
        fetch(`/api/view-education-level`)
            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    ...
                    membersTableBody.innerHTML = '';
                    data.forEach((EducationLevel, index) => {
                        const row = `
                            <tr>
                                <td class="text-center align-middle fw-bold">${EducationLevel.id}</td>
                                <td class="text-start align-middle">${EducationLevel.name_level}</td>
                                <td class="text-center align-middle">
                                    <button class="btn btn-view" onclick="ViewEducationLevel(${EducationLevel.id})">
                                    ...
                                    <button class="btn btn-edit" onclick="EditEducationLevel(${EducationLevel.id}, '${EducationLevel.name_level}')" data-bs-toggle="modal" data-bs-target="#EditEducationLevel">
                                ...
                                <button class="btn btn-block" onclick="DeleteEducationLevel(${EducationLevel.id})">
                                        <svg width="18" height="18" viewBox="-0.5 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg" onclick="DeleteEducationLevel(${EducationLevel.id}">
                                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                            <g id="SVGRepo_iconCarrier"> 
    ...
    function ViewEducationLevel(id) {
        // Construct the URL with the provided ID
        var url = `/education_course/${page_id}/${id}`;
    ...
    function AddEducationLevel () {
        const educationLevel = document.getElementById("EducationLevelName").value;

        // Send POST request to create a new Education Level
        fetch('/api/add-education-level', {
        method: 'POST',
        headers: {
        ...
        if(data == 0) {
            ...
            document.getElementById("EducationLevelName").value = ""; // Clear input field

            // Close modal using jQuery
            $('#AddEducationLevel').modal('hide');
    ...
    function EditEducationLevel(id, name) {
        document.getElementById("EditEducationLevelId").value = id;
        document.getElementById("EditEducationLevelName").value = name;
    }

    /* Update */
    function UpdateEducationLevel() {
        const educationLevelId = document.getElementById("EditEducationLevelId").value;
        const editedEducationLevel = document.getElementById("EditEducationLevelName").value;

        // Send PUT request to update the Education Level
        fetch(`/api/update-education-level/${educationLevelId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' // Add CSRF token if CSRF protection is enabled
            ...
            body: JSON.stringify({
                name_level: editedEducationLevel
            })
        })
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            // Handle success response
            if(data == 0) {
                document.getElementById("EditEducationLevelName").value = ""; // Clear input field
                document.getElementById("EditEducationLevelId").value = ""; // Clear input field

                // Close modal using jQuery
                $('#EditEducationLevel').modal('hide');
    ...
    function DeleteEducationLevel(id) {
        ...
        fetch(`/api/delete-education-level/${id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' // Add CSRF token if CSRF protection is enabled
...
<?php echo $__env->make('header.header', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Admin-IT\Desktop\tru-international\example-app\resources\views/education/educationlevel.blade.php ENDPATH**/ ?>